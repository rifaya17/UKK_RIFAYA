<?php
session_start();

// Cek login admin
if (!isset($_SESSION['admin'])) {
    header("Location: login-admin.php");
    exit();
}

// Set timezone Indonesia
date_default_timezone_set('Asia/Jakarta');

// Koneksi Database
$conn = mysqli_connect("localhost", "root", "", "dbukk_rifaya");

if (!$conn) {
    die("Koneksi gagal: " . mysqli_connect_error());
}

// Ambil semua filter dari URL
$filter_urutan = isset($_GET['urutan']) ? $_GET['urutan'] : 'terbaru';
$filter_status = isset($_GET['status']) ? $_GET['status'] : '';
$filter_kategori = isset($_GET['kategori']) ? $_GET['kategori'] : '';
$filter_tanggal_range = isset($_GET['tanggal_range']) ? $_GET['tanggal_range'] : '';
$filter_nis = isset($_GET['nis']) ? $_GET['nis'] : '';

// Buat WHERE clause
$where = [];
if ($filter_status) {
    if ($filter_status == 'Menunggu') {
        $where[] = "(asp.status = 'Menunggu' OR asp.status IS NULL)";
    } else {
        $where[] = "asp.status = '$filter_status'";
    }
}
if ($filter_kategori) {
    $where[] = "i.id_kategori = '$filter_kategori'";
}
if ($filter_tanggal_range) {
    $dates = explode(' to ', $filter_tanggal_range);
    if (count($dates) == 2) {
        $start_date = date('Y-m-d 00:00:00', strtotime($dates[0]));
        $end_date = date('Y-m-d 23:59:59', strtotime($dates[1]));
        $where[] = "i.tanggal BETWEEN '$start_date' AND '$end_date'";
    } elseif (count($dates) == 1) {
        $start_date = date('Y-m-d 00:00:00', strtotime($dates[0]));
        $end_date = date('Y-m-d 23:59:59', strtotime($dates[0]));
        $where[] = "i.tanggal BETWEEN '$start_date' AND '$end_date'";
    }
}
if ($filter_nis) {
    $filter_nis_escaped = mysqli_real_escape_string($conn, $filter_nis);
    $where[] = "i.nis LIKE '%$filter_nis_escaped%'";
}

$where_sql = count($where) > 0 ? "WHERE " . implode(" AND ", $where) : "";

// Tentukan urutan berdasarkan parameter
$order_by = "ORDER BY i.tanggal DESC"; // Default: terbaru
if ($filter_urutan == 'terlama') {
    $order_by = "ORDER BY i.tanggal ASC";
}

// Query dengan filter
$query = "SELECT 
            i.id_pelaporan,
            i.nis,
            i.id_kategori,
            i.lokasi,
            i.ket,
            i.tanggal,
            IFNULL(asp.status, 'Menunggu') as status,
            asp.feedback,
            IFNULL(k.ket_kategori, 'Tidak ada kategori') as ket_kategori
          FROM input_aspirasi i
          LEFT JOIN aspirasi asp ON i.id_pelaporan = asp.id_pelaporan
          LEFT JOIN kategori k ON i.id_kategori = k.id_kategori
          $where_sql
          $order_by";

$result = mysqli_query($conn, $query);

if (!$result) {
    die("Error query: " . mysqli_error($conn));
}

// Tanggal cetak
$tanggal_cetak = date('d/m/Y H:i:s');
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Laporan Aspirasi SMKN 12 Malang</title>
    <style>
        @media print {
            @page {
                size: A4 portrait;
                margin: 1.5cm;
            }
            body {
                margin: 0;
                padding: 0;
            }
        }
        
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }
        
        body {
            font-family: Arial, sans-serif;
            color: #000000;
            background: #ffffff;
        }
        
        .header {
            text-align: center;
            margin-bottom: 25px;
            border-bottom: 3px solid #000000;
            padding-bottom: 15px;
        }
        
        .header h2 {
            margin: 5px 0;
            font-size: 16pt;
            font-weight: bold;
            color: #000000;
        }
        
        .header p {
            margin: 5px 0;
            font-size: 10pt;
            color: #000000;
        }
        
        table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 15px;
        }
        
        table th {
            background-color: #ffffff;
            color: #000000;
            padding: 8px 6px;
            text-align: left;
            font-size: 9pt;
            border: 1px solid #000000;
            font-weight: 700;
        }
        
        table td {
            padding: 6px;
            border: 1px solid #000000;
            font-size: 8pt;
            vertical-align: top;
            color: #000000;
            background-color: #ffffff;
        }
        
        .status-badge {
            display: inline-block;
            padding: 3px 6px;
            border-radius: 3px;
            font-size: 7pt;
            font-weight: 700;
            border: 1px solid #000000;
            color: #000000;
            background-color: #ffffff;
        }
    </style>
</head>
<body>
    
    <div class="header">
        <h2>LAPORAN HASIL ASPIRASI SISWA<br>SMKN 12 MALANG</h2>
        <p>Tanggal Cetak: <?= $tanggal_cetak ?> WIB</p>
    </div>
    
    <table>
        <thead>
            <tr>
                <th width="3%">No</th>
                <th width="10%">Tanggal</th>
                <th width="8%">NIS</th>
                <th width="12%">Kategori</th>
                <th width="10%">Lokasi</th>
                <th width="27%">Keterangan</th>
                <th width="8%">Status</th>
                <th width="22%">Feedback</th>
            </tr>
        </thead>
        <tbody>
            <?php 
            $no = 1;
            while ($row = mysqli_fetch_assoc($result)) { 
                $tanggal = isset($row['tanggal']) ? date('d/m/Y H:i', strtotime($row['tanggal'])) : '-';
                $lokasi = !empty($row['lokasi']) ? htmlspecialchars($row['lokasi']) : '-';
                $keterangan = htmlspecialchars($row['ket']);
                $feedback = !empty($row['feedback']) ? htmlspecialchars($row['feedback']) : 'Belum ada feedback';
            ?>
            <tr>
                <td align="center"><?= $no++ ?></td>
                <td><?= $tanggal ?></td>
                <td align="center"><?= $row['nis'] ?></td>
                <td><?= $row['ket_kategori'] ?></td>
                <td><?= $lokasi ?></td>
                <td><?= $keterangan ?></td>
                <td align="center">
                    <span class="status-badge"><?= $row['status'] ?></span>
                </td>
                <td><?= $feedback ?></td>
            </tr>
            <?php } ?>
        </tbody>
    </table>
    
    <script>
        window.onload = function() { 
            window.print();
            
            // Detect ketika print dialog ditutup (baik print maupun cancel)
            window.onafterprint = function() {
                window.close();
            };
            
            // Fallback: jika window.close() tidak bekerja (beberapa browser), redirect ke dashboard
            setTimeout(function() {
                if (!window.closed) {
                    window.location.href = 'admin-dashboard.php';
                }
            }, 500);
        }
    </script>
</body>
</html>
<?php
mysqli_close($conn);
?>